...
現在の会話をMarkdownへ変換するだけでなく、必要な範囲の選択、引き継ぎ指示、差分エクスポートまでをローカルで行います。
...
- 会話ごとの最終ダウンロード位置だけをブラウザ内へ保存
- `verified`、`uncertain`、`incomplete`による取得品質の表示

通常のプレビューには、会話タイトル、取得元、メッセージ件数、取得品質だけを表示します。検索結果は検索時だけ表示し、内部取得経路の詳細は警告がある場合だけ利用者へ伝えます。
...
会話取得後のプレビュー画面にある「使い方」で、Markdownプレビューと利用ガイドを切り替えられます。Chromeの拡張機能詳細画面にある「拡張機能のオプション」から、ガイドだけを直接開くこともできます。
...
会話本文は`chrome.storage.local`へ保存しません。プレビュー中はbackgroundのメモリに一時保持します。「今回の位置を記憶する」が有効な場合、会話ごとに次の情報だけを保存します。
...
この情報は端末間で同期されず、Chromeプロファイルごとに管理されます。拡張を削除すると消える可能性がありますが、ダウンロード済みMarkdownには影響しません。
...
- `popup/`: 抽出開始UI
- `preview/`: 範囲選択、差分出力、Markdownプレビュー
